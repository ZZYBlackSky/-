                ...
                success: function (res) {
                    ...
                    for (var i = pageStart; i < pageEnd; i++) {
                        ...
                        result += '<li class="activity_vote_AnchorsContent">' +
                            ...
                            '<p class="activity_votes">投票数：<span id="span_' + res[i].id + '">' + res[i].piao + '</span>票</p>' +
                            '</div>' +
                            '<img class="activity_user_head" src="http://wt.cctbn.com/tpl/1202/images/' + res[i].portrait + '"/>' +
                            '</div>' +
                            '<button class="submit_vote" type="submit">投票</button>' +
...
//投票					
                        ...
                        $(".submit_vote").on("click", function (e) {
                            ...
                            var uid = $('#uid').val();
                            var xx = $(this).prev().find("p").eq(3).find('span').text();

                            $.ajax({
                                type: "get",
                                dataType: 'json',
                                url: "/index.php?g=1202&m=Index&a=vote",
                                async: true,
                                data: {uid: uid, works_id: userId},
                                success: function (res) {
                                    console.log(res);
                                    if (res.flag == "y") {
                                        var xxx = parseInt(xx) + 1;
                                        var newPiao = '#span_' + userId;
                                        $(newPiao).text(xxx);
